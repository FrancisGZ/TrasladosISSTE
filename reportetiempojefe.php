...
function FancyTable($header)
{
    ...
    $this->SetFont('','B');
    //Header
    $w=array(13,13,16,13,13,8,13,13,13,13,13,13,20,15);
...
$conexion = pg_connect("host=localhost dbname=iste user=postgres password=administrador") or
   ...
   $result = pg_query ($conexion, " select  * from reportetiempo where Fecha BETWEEN '$_GET[nocedula]' AND '$_GET[nocedula1]'")
...
$pdf=new PDF();
//Column titles
$header=array('Destino','NoOrden','Cedula','Tipo','Sexo','Edad','Nombre','Ap Paterno','Ap Materno','Sub Pasajero','Fecha','Total','Clinica','Especialidad');
$pdf->SetFont('Arial','',5);
$pdf->AddPage();
