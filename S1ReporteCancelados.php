...
if ($boton =="Imprimir" )
   {
  header("location:S1pdfCancelados.php?tipo=$radio&suma=$suma&total=$total");
  }
...
<form id="form1" name="form1" method="post" action="S1ReporteCancelados.php">
   ...
   if ($boton=="Buscar")
{
            ...
            if ($radio=="grl")
             {
               ...
               $result = pg_query ($conexion, "SELECT * from reportecancelado  where unidadp='$_SESSION[umf]' and estado='CANCELADO'")
             ...
             $res = pg_query ($conexion, "select sum(total) as total from reportecancelado where unidadp='$_SESSION[umf]' and estado='CANCELADO'")

             or die("Error en la consulta SQL");
             
            
            
            $r = pg_query ($conexion, "select count(total) as tras from reportecancelado where unidadp='$_SESSION[umf]' and estado='CANCELADO'")
             ...
             $result = pg_query ($conexion, "SELECT *from reportecancelado where subac not in (0.00)  and unidadp='$_SESSION[umf]' and estado='CANCELADO'")

             or die("Error en la consulta SQL");
             
            $res = pg_query ($conexion, "select sum(total) as total from reportecancelado where subac not in (0.00) and unidadp='$_SESSION[umf]' and estado='CANCELADO'")

             or die("Error en la consulta SQL");
			 
			 
			 $r = pg_query ($conexion, "select count(total) as tras from reportecancelado where subac not in (0.00) and unidadp='$_SESSION[umf]' and estado='CANCELADO'")
            ...
            $result = pg_query ($conexion, "SELECT * from reportecancelado where subac=0.00 and unidadp='$_SESSION[umf]' and estado='CANCELADO'")

             or die("Error en la consulta SQL");



             $res = pg_query ($conexion, "select sum(total) as total from reportecancelado where subac=0.00 and unidadp='$_SESSION[umf]' and estado='CANCELADO'")

             or die("Error en la consulta SQL");
			 
			 
			 
             $r = pg_query ($conexion, "select count(total) as tras from reportecancelado where subac=0.00 and unidadp='$_SESSION[umf]' and estado='CANCELADO'")
 ...
 echo "<table class='tabla' align='center'  border='1' bgcolor='#FF9900' >
   <tr >
     <td align='center'>No. Orden </div></td>
     <td align='center' width='50'>Fecha</div></td>
