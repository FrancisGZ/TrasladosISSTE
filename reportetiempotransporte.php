...
$conexion = pg_connect("host=localhost dbname=iste user=postgres password=administrador") or
   ...
   $result = pg_query ($conexion, " select  * from reportetiempotransporte where nomunidad='$_SESSION[umf]' and Fecha BETWEEN '$_GET[nocedula]' AND '$_GET[nocedula1]' order by fecha")
...
//Column titles
$header=array('Id Orden','Fecha','Cedula','Nombre','Ap Paterno','Ap Materno','Origen','Destino','Costo','Num Ambulancia','Chofer','Camillero','Medico','Elaboro');
$pdf->SetFont('Arial','',5);
$pdf->AddPage();
