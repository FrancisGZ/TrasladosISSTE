...
if($_GET["tipo"]=="grl")
{
   $result = pg_query ($conexion, "select * from reporte where unidadp='$_GET[umf]'" )
...
if ($_GET["tipo"]=="ac")
{
 ...
 $result = pg_query ($conexion, "SELECT * from reporte where subac not in (0.00) and unidadp='$_GET[umf]'")
...
$result = pg_query ($conexion, "SELECT * from reporte where subac=0.00 and unidadp='$_GET[umf]'")
...
//Column titles
$header=array('No. Orden','Fecha','Cedula','Tipo','Sexo','Edad','Nombre','Ap Paterno','Ap Materno','Categoria','Destino','Sub Pasajero','Nomb Acomp','Ap Paterno','Ap Materno','Categoria','Destino','Sub Acomp','Total','Especialidad','Clinica','Elaboro');
$pdf->SetFont('Arial','',4);
$pdf->AddPage();
