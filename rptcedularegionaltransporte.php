...
if ($boton =="Imprimir" )
   {
header ("location:rptcedutransportejefe.php?cedula=$cedula");
  }
...
<form id="form1" name="form1" method="post" action="rptcedularegionaltransporte.php">
   ...
   if ($boton=="Buscar")
{
      ...
      die ("Fallo en el establecimiento de la conexi&oacute;n");
            ...
            $result = pg_query ($conexion, "select idordentrans,fecha,cedula,nompasajero,appaterno,apmaterno,nomciudad,costo,nomunidad from reportetransporte where cedula = '$cedula'")

             or die("Error en la consulta SQL");


              $res = pg_query ($conexion, "select nompasajero,appaterno,apmaterno from reportetransporte where cedula = '$cedula'")

             or die("Error en la consulta SQL");


            $re = pg_query ($conexion, "select sum(costo) as total from reportetransporte where cedula = '$cedula'")

             or die("Error en la consulta SQL");

           

             $r = pg_query ($conexion, "select count(costo) as tras from reportetransporte where cedula = '$cedula'")
