...
if ($boton =="Imprimir" )
   {
  header("location:S1pdfMF.php?tipo=$radio&suma=$suma&total=$total&fecha1=$fecha1&fecha2=$fecha2");
  }

?>

<div id="contenedor" align="center">
<form id="form1" name="form1" method="post" action="A1ReporteMF.php">
...
<label>
 ...
 {
    echo "<input name='tipo' type='radio' value='ac' checked />Masculino";
}
if ($radio!="ac")
{
   echo    "<input name='tipo' type='radio' value='ac' />Masculino";
}
?>
     </label>
...
<? if ($radio=="grl") 
 ...
 {
    echo "<input name='tipo' type='radio' value='grl' checked />Femenino";
}
if ($radio!="grl")
{
   echo    "<input name='tipo' type='radio' value='grl' />Femenino";
}
?>
      </label>
    </div></td>
            ...
            if ($radio=="grl")
             {
               ...
               $result = pg_query ($conexion, "SELECT * from reportecancelado  where unidadp='$_SESSION[umf]' and estado='APROBADO' and sexo='FEMENINO' and Fecha BETWEEN '$fecha1' AND '$fecha2' order by fecha")
             ...
             $res = pg_query ($conexion, "select sum(total) as total from reportecancelado where unidadp='$_SESSION[umf]' and estado='APROBADO' and sexo='FEMENINO' and Fecha BETWEEN '$fecha1' AND '$fecha2'")

             or die("Error en la consulta SQL");
             
            
            
            $r = pg_query ($conexion, "select count(total) as tras from reportecancelado where unidadp='$_SESSION[umf]' and estado='APROBADO' and sexo='FEMENINO' and Fecha BETWEEN '$fecha1' AND '$fecha2'")
             ...
             $result = pg_query ($conexion, "SELECT *from reportecancelado where unidadp='$_SESSION[umf]' and estado='APROBADO' and sexo='MASCULINO'  and Fecha BETWEEN '$fecha1' AND '$fecha2' order by fecha")

             or die("Error en la consulta SQL");
             
            $res = pg_query ($conexion, "select sum(total) as total from reportecancelado where unidadp='$_SESSION[umf]' and estado='APROBADO' and sexo='MASCULINO' and Fecha BETWEEN '$fecha1' AND '$fecha2'")

             or die("Error en la consulta SQL");
			 
			 
			 $r = pg_query ($conexion, "select count(total) as tras from reportecancelado where unidadp='$_SESSION[umf]' and sexo='MASCULINO' and estado='APROBADO' and Fecha BETWEEN '$fecha1' AND '$fecha2'")
